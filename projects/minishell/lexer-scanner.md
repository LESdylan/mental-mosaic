...
1. Simplifies Parsing: By converting raw text into tokens, the lexer reduces the complexity of the [[Parser]]'s job The parser works with structured tokesn rather than raw characters, making it easier to analyze the code's syntax
...
1. Read input: The lexer reads the source code as a string tracking its position (e.g., linej and column numbers for error reporting)
...
4. `Advance and repeat` : The lexer advances its position in the input string and continues unitl it reaches the end fo the input or encounters an error.
### Implementingj a lexer in C
